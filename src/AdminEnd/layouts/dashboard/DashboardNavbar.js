...
import {getTableData, getTableSingle} from "../../../ApiServices/getData";
...
export default function DashboardNavbar({ onOpenSidebar,roleId }) {
  ...
  useEffect(async () => {
    const response = await getTableSingle('admin',roleId);
    ...
    <RootStyle>
      <ToolbarStyle>
        ...
        <Stack direction="row" alignItems="center" spacing={{ xs: 0.5, sm: 1.5 }}>
          <IconButton
              size="large"
              LinkComponent={Link} to={'/admin/dashboard/'+roleId+'/chat/'+data.fullname+'/'+roleId}
              sx={{
                ...(open && {
